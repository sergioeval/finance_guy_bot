...
def init_db():
    ...
    with get_connection() as conn:
        ...
        """)
        # Migración: agregar transfer_id si no existe
        try:
            conn.execute("ALTER TABLE transacciones ADD COLUMN transfer_id TEXT")
        except sqlite3.OperationalError:
            pass  # La columna ya existe
        # Migración: agregar categoria si no existe
        try:
...
def registrar_gasto(user_id: int, nombre_cuenta: str, monto: float, categoria: str = "sin_categoria") -> tuple[bool, str]:
    ...
    # Para cuentas de crédito, un gasto aumenta la deuda (saldo negativo)
    # Para cuentas de débito, un gasto disminuye el saldo
    with get_connection() as conn:
        if cuenta["tipo"] == "debito":
            nuevo_saldo = cuenta["saldo"] - monto
        else:
            nuevo_saldo = cuenta["saldo"] - monto  # crédito: más negativo = más deuda
...
def transferir(user_id: int, cuenta_origen: str, cuenta_destino: str, monto: float) -> tuple[bool, str]:
    ...
    # Verificar saldo suficiente en cuenta de débito
...
def obtener_resumen(user_id: int) -> dict:
    ...
    return {
        ...
        "patrimonio_neto": total_debito + total_credito,  # crédito suele ser negativo
    }
...
def obtener_resumen_por_categoria(
    ...
    Opcionalmente filtra por año y/o mes.
    Retorna: { "gastos": [{categoria, total}], "ingresos": [...], "total_gastos", "total_ingresos" }
    """
    with get_connection() as conn:
        filtro = "AND user_id = ?"
        params: list = [user_id]
...
def obtener_resumen_por_mes(
    ...
    - Sin args: últimos N meses.
    - Solo ano: todos los meses de ese año.
    - ano y mes: solo ese mes.
    Retorna lista de {ano, mes, gastos, ingresos, balance}
    """
    with get_connection() as conn:
        if ano is not None and mes is not None:
            rows = conn.execute("""
                SELECT
                    CAST(strftime('%Y', creada_en) AS INTEGER) AS ano,
...
def listar_registros(user_id: int, nombre_cuenta: str) -> tuple[list[dict] | None, str]:
    ...
    Retorna (lista de transacciones, mensaje) o (None, mensaje_error).
    """
    cuenta = obtener_cuenta_por_nombre(user_id, nombre_cuenta)
    if not cuenta:
...
def editar_registro(
    ...
    """
    Edita un gasto o ingreso. Solo permite editar monto y/o categoría.
    Retorna (éxito, mensaje).
    """
    ...
    with get_connection() as conn:
        ...
        if monto is not None:
            # Ajustar saldo: revertir efecto del monto viejo, aplicar el nuevo
            if tipo == "gasto":
...
def eliminar_registro(user_id: int, transaccion_id: int) -> tuple[bool, str]:
    """
    Elimina una transacción y revierte el saldo de la(s) cuenta(s) afectada(s).
    Para transferencias, elimina ambos registros del par.
    Retorna (éxito, mensaje).
    """
    trans = obtener_transaccion(user_id, transaccion_id)
    if not trans:
    ...
    with get_connection() as conn:
        ...
        if tipo == "gasto":
            ...
            # Buscar el par de la transferencia
            ...
            par = dict(par)
            # Origen = cuenta que perdió dinero; Destino = cuenta que ganó
